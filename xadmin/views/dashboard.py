...
class PartialBaseWidget(BaseWidget):
	...
	def make_get_request(self, path, data=None, **extra):
		req = self.get_factory().get(path, data, **extra)
		return self.setup_request(req)

	def make_post_request(self, path, data=None, **extra):
...
class ListWidget(ModelBaseWidget, PartialBaseWidget):
	...
	def setup(self):
		...
		self.list_view = self.get_view_class(ListAdminView, self.model)()
		self.list_view.setup(req)
